...
struct msg_pingreq : public message_header {
    ...
    void init_msg_pingreq(msg_pingreq *pingreq, const char *client_id) {
        uint8_t client_id_length = 0;
        if (client_id != nullptr) {
            client_id_length = (uint8_t) strlen(client_id);
            if (client_id_length > 22) {
                client_id_length = 23;
            }
        }

        //TODO this should also be in the Gateway Stuff
        if (client_id_length == 0) {
            // if now client_id given use only length of 2
            pingreq->length = ((uint8_t) 2);
        } else {
